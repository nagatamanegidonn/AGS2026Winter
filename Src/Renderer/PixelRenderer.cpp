...
void PixelRenderer::MakeSquereVertex(Vector2 pos, Vector2 size)
{
	...
	int cnt = 0;
	float sX = static_cast<float>(pos.x - size.x / 2);
	float sY = static_cast<float>(pos.y - size.y / 2);
	float eX = static_cast<float>(pos.x + size.x / 2);
	float eY = static_cast<float>(pos.y + size.y / 2);
