...
bool AsoUtility::IsLenge(const VECTOR& sPos, const VECTOR& ePos, float lenge)
{
...
float AsoUtility::GetDisPow(const VECTOR& sPos, const VECTOR& ePos)
{
    VECTOR diff = VSub(sPos, ePos);
    float disPow = diff.x * diff.x + diff.y * diff.y + diff.z * diff.z;
...
float AsoUtility::CalcVolumeByDistance(const VECTOR& listener, const VECTOR& source, float maxDistance)
{
