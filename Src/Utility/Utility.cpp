...
VECTOR Utility::MV1GetFreamPos(const int& modelId, const std::wstring& freamName)
{
    auto frmNo = MV1SearchFrame(modelId, freamName.c_str());
    // プレイヤーの手の位置
    auto posHead = MV1GetFramePosition(modelId, frmNo);
...
{
    float diff = fmodf(to - from + DX_PI, DX_TWO_PI) - DX_PI;
    return from + diff * t;
}

bool Utility::IsLenge(const VECTOR& sPos, const VECTOR& ePos, float lenge)
{
    VECTOR diff = VSub(sPos, ePos);
...
float Utility::GetDisPow(const VECTOR& sPos, const VECTOR& ePos)
{
    VECTOR diff = VSub(sPos, ePos);
    float disPow = diff.x * diff.x + diff.y * diff.y + diff.z * diff.z;
...
float Utility::CalcVolumeByDistance(const VECTOR& listener, const VECTOR& source, float maxDistance)
{
    float disPow = GetDisPow(listener, source);
    float maxDisPow = maxDistance * maxDistance;
    float volume = 1.0f - (disPow / maxDisPow);
    return std::clamp(volume, 0.0f, 1.0f);
}

std::string Utility::WStringToUTF8_Win(const std::wstring& wstr)
{
...
std::wstring Utility::UTF8ToWString_Win(const std::string& str)
{
